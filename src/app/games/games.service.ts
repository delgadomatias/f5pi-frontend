...
export class GamesService {
  ...
  getGames(params: PaginatedRequest = DEFAULT_PAGINATION_PARAMS) {
    ...
    return this.http.get<GamesResponse>(`${environment.API_URL}/api/v1/users/${userId}/games`, {
      params: { ...params, pageSize: 20 },
    });
  }

  getGameDetail(gameId: Game['gameId']) {
    return this.http.get<GameDetailResponse>(`${environment.API_URL}/api/v1/games/${gameId}/detail`);
  }
  ...
  createGame(request: CreateGameRequest) {
    const userId = this.authService.getUserId();
    return this.http.post<Game>(`${environment.API_URL}/api/v1/games`, {
      ...request,
      userId,
    });
  }
  ...
  createGameDetail(gameId: Game['gameId'], request: CreateGameDetailRequest) {
    return this.http.post<GameDetailResponse>(`${environment.API_URL}/api/v1/games/${gameId}/detail`, request);
  }

  deleteGame(gameId: Game['gameId']) {
    return this.http.delete<void>(`${environment.API_URL}/api/v1/games/${gameId}`);
  }

  updateGame(gameId: Game['gameId'], request: UpdateGameRequest) {
    return this.http.patch<Game>(`${environment.API_URL}/api/v1/games/${gameId}`, request);
  }
